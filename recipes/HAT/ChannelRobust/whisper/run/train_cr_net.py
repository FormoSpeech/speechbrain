...
class CRNet(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        total_loss_mse = 0
        loss_cnt = 0
        channel_num_per_stem = 7
        target_index = 6
        
        if self.hparams.condenser_remixed == False:
            selected_indices = [j for j in range(channel_num_per_stem) if j != target_index]
            remix_num = 6
        else:
            selected_indices = [j for j in range(channel_num_per_stem)]
            remix_num = 7
        ...
        for i in range(0, len(channel_ids), channel_num_per_stem):
            ...
            # Process whole channels
            wavs_all = wavs_stem[selected_indices]
            bos_tokens_other = bos_tokens_stem[selected_indices]
            enc_out, _, _ = self.modules.whisper(wavs_all, bos_tokens_other)
            
            # 7 * T * D 
            enc_out_last = enc_out[-1]
            
            weights = torch.softmax(torch.randn(remix_num, device=enc_out_last.device), dim=0).view(remix_num, 1, 1)
            remixed_enc_out = (enc_out_last * weights).sum(dim=0, keepdim=True)
                     
            if self.hparams.gaussian_blur == True:   
            # # Apply Gaussian filter and convert to torch.Tensor
                gaussian_blurred_embedding_np = gaussian_filter(remixed_enc_out.cpu().numpy(), sigma=torch.rand(1).item()*2)
                gaussian_blurred_embedding = torch.tensor(gaussian_blurred_embedding_np, device=remixed_enc_out.device)

                # Ensure the tensor is of the correct type and device
                remixed_enc_out = gaussian_blurred_embedding.to(remixed_enc_out.dtype)
                
            # Pool the encoder outputs for classifier input
            cr_embeddings = self.modules.crNet(remixed_enc_out)
            
            # Calculate the loss_mse between the target output and the other channels' output
            loss_mse = self.modules.mse(cr_embeddings, enc_out_ref_last)
                                
            # Accumulate losses
            total_loss_mse += loss_mse
            loss_cnt += 1
            
        final_loss_mse = total_loss_mse / loss_cnt * 1000
        
        final_loss = final_loss_mse 
        return final_loss
...
if __name__ == "__main__":
    ...
    if "pretrainer" in hparams.keys():
        ...
        hparams["pretrainer"].load_collected(crNet.device)

    # We dynamically add the tokenizer to our brain class.
    # NB: This tokenizer corresponds to the one used for Whisper.
    crNet.tokenizer = tokenizer
    ...
    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        ...
        crNet.hparams.test_channel_file = os.path.join(
